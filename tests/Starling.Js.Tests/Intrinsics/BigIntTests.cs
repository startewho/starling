...
{
    // ----------------------------------------------------------- Literals
    [Fact] public void Literal_decimal_zero() =>
        Eval("0n === 0n").AsBool.Should().BeTrue();

    [Fact] public void Literal_decimal_addition() =>
        Eval("123n + 1n === 124n").AsBool.Should().BeTrue();

    [Fact] public void Literal_hex() =>
        Eval("0xFFn === 255n").AsBool.Should().BeTrue();

    [Fact] public void Literal_binary() =>
        Eval("0b101n === 5n").AsBool.Should().BeTrue();

    [Fact] public void Literal_octal() =>
        Eval("0o17n === 15n").AsBool.Should().BeTrue();

    // ------------------------------------------------------------- typeof
    [Fact] public void Typeof_bigint() =>
        Eval("typeof 1n").AsString.Should().Be("bigint");

    // ------------------------------------------------------- BigInt(value)
    [Fact] public void Ctor_from_number() =>
        Eval("BigInt(5) === 5n").AsBool.Should().BeTrue();

    [Fact] public void Ctor_from_decimal_string() =>
        Eval("BigInt('100') === 100n").AsBool.Should().BeTrue();

    [Fact] public void Ctor_from_hex_string() =>
        Eval("BigInt('0x10') === 16n").AsBool.Should().BeTrue();

    [Fact] public void Ctor_from_binary_string() =>
        Eval("BigInt('0b1010') === 10n").AsBool.Should().BeTrue();

    [Fact] public void Ctor_from_octal_string() =>
        Eval("BigInt('0o17') === 15n").AsBool.Should().BeTrue();

    [Fact] public void Ctor_from_boolean_true() =>
        Eval("BigInt(true) === 1n").AsBool.Should().BeTrue();

    [Fact] public void Ctor_from_boolean_false() =>
        Eval("BigInt(false) === 0n").AsBool.Should().BeTrue();

    [Fact] public void Ctor_from_bigint_identity() =>
    ...
    [Fact] public void Subtract() => Eval("5n - 3n === 2n").AsBool.Should().BeTrue();
    [Fact] public void Multiply() => Eval("4n * 6n === 24n").AsBool.Should().BeTrue();
    [Fact] public void Divide_truncates_toward_zero() => Eval("10n / 3n === 3n").AsBool.Should().BeTrue();
    [Fact] public void Divide_negative_truncates() => Eval("(-10n) / 3n === -3n").AsBool.Should().BeTrue();
    ...
    [Fact] public void BitOr() => Eval("(0xF0n | 0x0Fn) === 0xFFn").AsBool.Should().BeTrue();
    [Fact] public void BitXor() => Eval("(0xFFn ^ 0x0Fn) === 0xF0n").AsBool.Should().BeTrue();
    [Fact] public void BitNot() => Eval("~0n === -1n").AsBool.Should().BeTrue();
    [Fact] public void ShiftLeft() => Eval("1n << 8n === 256n").AsBool.Should().BeTrue();
    ...
    [Fact] public void Strict_equal_different_kind_is_false() => Eval("1n === 1").AsBool.Should().BeFalse();
    [Fact] public void Loose_equal_to_number_integer() => Eval("1n == 1").AsBool.Should().BeTrue();
    [Fact] public void Loose_equal_to_number_fractional_is_false() => Eval("1n == 1.5").AsBool.Should().BeFalse();
    [Fact] public void Loose_equal_to_string() => Eval("1n == '1'").AsBool.Should().BeTrue();
    [Fact] public void Loose_unequal_to_unparseable_string() => Eval("1n == 'abc'").AsBool.Should().BeFalse();

    // ---------------------------------------------------------- Comparison
    [Fact] public void Compare_gt_bigint_bigint() => Eval("2n > 1n").AsBool.Should().BeTrue();
    [Fact] public void Compare_lt_bigint_number_integer() => Eval("1n < 2").AsBool.Should().BeTrue();
    [Fact] public void Compare_lt_bigint_number_fractional() => Eval("1n < 2.5").AsBool.Should().BeTrue();
    [Fact] public void Compare_lt_bigint_number_just_below() => Eval("2n < 1.5").AsBool.Should().BeFalse();

    // ---------------------------------------------------------- toString
    [Fact] public void Proto_toString_default_decimal() =>
        Eval("(255n).toString()").AsString.Should().Be("255");

    [Fact] public void Proto_toString_hex() =>
        Eval("(255n).toString(16)").AsString.Should().Be("ff");

    [Fact] public void Proto_toString_binary() =>
        Eval("(5n).toString(2)").AsString.Should().Be("101");

    [Fact] public void Proto_valueOf_returns_bigint() =>
        Eval("typeof (1n).valueOf()").AsString.Should().Be("bigint");

    // ----------------------------------------------------- asIntN / asUintN
    [Fact] public void AsIntN_8_negative_via_sign_bit() =>
        Eval("BigInt.asIntN(8, 200n) === -56n").AsBool.Should().BeTrue();

    [Fact] public void AsIntN_8_positive() =>
        Eval("BigInt.asIntN(8, 100n) === 100n").AsBool.Should().BeTrue();

    [Fact] public void AsUintN_8_overflow_wraps_to_zero() =>
