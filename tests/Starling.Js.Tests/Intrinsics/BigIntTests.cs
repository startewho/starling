...
public class BigIntTests
{
    // ----------------------------------------------------------- Literals
    [TestMethod]
    public void Literal_decimal_zero() =>
        Eval("0n === 0n").AsBool.Should().BeTrue();

    [TestMethod]
    public void Literal_decimal_addition() =>
        Eval("123n + 1n === 124n").AsBool.Should().BeTrue();

    [TestMethod]
    public void Literal_hex() =>
    ...
    [TestMethod]
    public void Negative_exponent_throws() =>
    ...
    // ---------------------------------------------------------- Unary
    [TestMethod]
    public void Unary_minus_works() =>
        Eval("(-5n) === 0n - 5n").AsBool.Should().BeTrue();

    [TestMethod]
    public void Unary_plus_throws() =>
        ((Action)(() => Eval("+1n"))).Should().Throw<JsThrow>();

    // ---------------------------------------------------------- Mixed-type
    [TestMethod]
    public void Mixed_add_throws() =>
        ((Action)(() => Eval("1n + 1"))).Should().Throw<JsThrow>();

    [TestMethod]
    public void Mixed_multiply_throws() =>
        ((Action)(() => Eval("1n * 2"))).Should().Throw<JsThrow>();

    [TestMethod]
    public void Mixed_divide_throws() =>
        ((Action)(() => Eval("4n / 2"))).Should().Throw<JsThrow>();

    [TestMethod]
    public void Unsigned_shift_on_bigint_throws() =>
        ((Action)(() => Eval("1n >>> 0n"))).Should().Throw<JsThrow>();
