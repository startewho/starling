...
| CSSOM View | https://drafts.csswg.org/cssom-view/ | 🟢 | `CssomView1/` + `CssomViewBindingTests` (Starling JS engine) — `DOMRectReadOnly`/`DOMRect` geometry; JS `getBoundingClientRect`→DOMRect shape (+`toJSON`), `getClientRects`, box-metric accessors (incl. `scrollTop`/`scrollLeft`), `matchMedia`→MediaQueryList all exposed + tested on the default engine (`wp:spec-cssom-view`); real scroll offsets + `scrollIntoView` + live media-change events remain | `wp:spec-cssom-view` |
