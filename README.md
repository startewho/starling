...
| **JS engine** | 🟡 | Lex + parse (full ES2024 grammar) + bytecode compiler + register VM. Intrinsics (Object/Array/String/Number/Boolean/Symbol/BigInt/Math/JSON/Date/RegExp/Map/Set/WeakMap/WeakSet/WeakRef/Proxy/Reflect/ArrayBuffer/DataView/TypedArrays/Error), Promise + microtasks, async/await, classes, generators, destructuring (declarations, parameters, **and assignment**), and **ES modules** (compile + loader + `<script type="module">`) all work. Still ahead: top-level await, dynamic `import()`/`import.meta`, async generators (currently stubbed), Test262 ≥ 80%. |
