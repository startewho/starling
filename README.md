...
| [**JS engine**](browser-plan/09_JS_ENGINE.md) | 🟡 | Full ES2024 parse → bytecode → register VM: intrinsics, Promise + async/await, generators, classes, ES modules (top-level await, dynamic `import()`). Test262 `language` at 41.23% (target ≥ 80%). |
