...
| **JS engine** | 🟡 | Lex + parse (full ES2024 grammar) + bytecode compiler + register VM. Intrinsics (Object/Array/String/Number/Boolean/Symbol/BigInt/Math/JSON/Date/RegExp/Map/Set/WeakMap/WeakSet/WeakRef/Proxy/Reflect/ArrayBuffer/DataView/TypedArrays/Error), Promise + microtasks, async/await, **async generators** (incl. `for await…of`), classes (incl. **computed member keys** + `[Symbol.iterator]`), generators, destructuring (declarations, parameters, assignment, **and module-scope binding patterns**), and **ES modules** (compile + loader + `<script type="module">`) all work. Still ahead: top-level await, dynamic `import()`/`import.meta`, Test262 ≥ 80%. |
