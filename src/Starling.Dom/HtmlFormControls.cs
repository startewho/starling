...
public static class HtmlFormControls
{
    ...
    {
        if (element.LocalName == "select") SelectOptionByValue(element, value);
        else element.InputValue = value;
    ...
    public static void SetChecked(Element element, bool value)
    {
        ...
        if (InputType(element) != "radio") return;
        ...
        foreach (var other in element.OwnerDocument?.DescendantElements() ?? Root(element).DescendantElements())
        {
            ...
            if (other.LocalName != "input" || InputType(other) != "radio") continue;
            if (!string.Equals(other.GetAttribute("name") ?? string.Empty, name, StringComparison.Ordinal)) continue;
            if (!ReferenceEquals(FormOwner(other), owner)) continue;
    ...
    public static Element? FormOwner(Element element)
    {
        ...
        for (Node? n = element.ParentNode; n is not null; n = n.ParentNode)
            if (n is Element { LocalName: "form" } form) return form;
        return null;
    }

    public static IReadOnlyList<Element> FormControls(Element form)
    {
        if (form.LocalName != "form") return Array.Empty<Element>();
        ...
        if (form.OwnerDocument is not null && !string.IsNullOrEmpty(id))
        {
            foreach (var element in form.OwnerDocument.DescendantElements())
            {
                ...
                if (!string.Equals(element.GetAttribute("form"), id, StringComparison.Ordinal)) continue;
    ...
    {
        if (form.LocalName != "form") return Array.Empty<FormEntry>();
    ...
    public static bool CheckValidity(Element element)
    {
        if (element.LocalName == "form")
        {
            foreach (var control in FormControls(element))
                if (!Validity(control).Valid) return false;
            return true;
        }
    ...
    {
        if (!string.IsNullOrEmpty(element.CustomValidationMessage)) return element.CustomValidationMessage;
        var validity = Validity(element);
        if (validity.Valid) return string.Empty;
        if (validity.ValueMissing) return "Please fill out this field.";
        if (validity.TypeMismatch) return "Please enter a valid value.";
        if (validity.PatternMismatch) return "Please match the requested format.";
        if (validity.TooLong) return "Please shorten this text.";
        if (validity.TooShort) return "Please lengthen this text.";
        if (validity.RangeUnderflow) return "Please enter a larger value.";
        if (validity.RangeOverflow) return "Please enter a smaller value.";
    ...
    {
        if (form.LocalName != "form" || form.OwnerDocument is not { } document) return;
        foreach (var control in FormControls(form))
        {
            if (!AllowsAutocomplete(control)) continue;
    ...
    private static void AppendEntries(List<FormEntry> entries, Element control, Element? submitter)
    {
        ...
        switch (control.LocalName)
        {
            case "input":
                ...
                if (type is "button" or "reset" or "file" or "image") return;
                if (type == "submit" && !ReferenceEquals(control, submitter)) return;
                if (type is "checkbox" or "radio")
                {
                    if (!Checked(control)) return;
    ...
    {
        if (string.IsNullOrEmpty(value) || element.LocalName != "input") return false;
    ...
    {
        if (element.LocalName != "input" || InputType(element) != "number") return false;
        if (!double.TryParse(value, NumberStyles.Float, CultureInfo.InvariantCulture, out var number)) return false;
        if (!double.TryParse(element.GetAttribute(attr), NumberStyles.Float, CultureInfo.InvariantCulture, out var limit)) return false;
    ...
    private static IEnumerable<Element> SelectedOptions(Element select)
    {
        ...
        foreach (var option in select.DescendantElements())
        {
            ...
            if (option.HasAttribute("selected"))
            {
                any = true;
                yield return option;
                if (!select.HasAttribute("multiple")) yield break;
            }
        }
        if (!any && first is not null) yield return first;
    }
    ...
    {
        var root = node;
        while (root.ParentNode is not null) root = root.ParentNode;
        return root;
    }
}
