...
public class UrlBench
{
    ...
    [Benchmark]
    public TUrl Simple_absolute()
        => UrlParser.Parse(Simple).Value;

    [Benchmark]
    public TUrl Pathy_with_query_and_fragment()
        => UrlParser.Parse(Pathy).Value;

    [Benchmark]
    public TUrl Idna_unicode_host()
        => UrlParser.Parse(Idna).Value;

    [Benchmark]
    public TUrl Ipv6_with_port()
        => UrlParser.Parse(Ipv6).Value;

    [Benchmark]
    public TUrl Percent_encoded_path_and_query()
        => UrlParser.Parse(Percent).Value;

    [Benchmark]
    public TUrl Relative_against_base()
        => UrlParser.Parse("../other/page.html?x=1", _baseUrl).Value;

    [Benchmark]
    public TUrl Relative_absolute_path()
